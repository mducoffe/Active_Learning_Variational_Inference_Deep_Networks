...
def increase_train_with_noise(train, gaussian_noise=0.1):
	x_train, y_train = train
	x_new, y_new=[]
	shape = x_train[0].shape
	for i in range(len(y_train)):
		noise = np.random.ranf(
...
def build_model():
	...
	L_pool_size = [None, (2,2)]
	L_activation_conv = ['rectifier', 'rectifier']
	L_dim_full_layers=[300, 50]
...
def training_committee_member(instance, learning_rate, train, batch_size, valid, valid_full=1.):
	...
	#w = W[0]; # no restriction on the parameters to train for normal qbc
	layer_number=None
	for w_tmp in W:
	...
	x_train, y_train = increase_train_with_noise(train, gaussian_noise=0.1)
