...
def RMSProp(cost, params, lr=0.002, decay_rate=0.4):
...
def build_model():
	num_channels = 3
	image_size = (32,32)
	#L_dim_conv_layers = [16,20]
	L_dim_conv_layers = [16,16]
	L_filter_size = [(5,5), (7,7)]
	#L_filter_size = [(3,3), (5,5)]
	L_pool_size = [ (2,2), (2,2)]
	L_activation_conv = ['rectifier', 'rectifier']
	L_dim_full_layers=[200]
...
"""
def training_committee_member(instance, learning_rate, train, batch_size):
	x = T.tensor4('x')
	y = T.imatrix('y')
	y_prev = instance.apply(x)
	#cost = CategoricalCrossEntropy().apply(y.flatten(), y_prev).mean()
	cost = Softmax().categorical_cross_entropy(y.flatten(), y_prev).mean()
    	error = MisclassificationRate().apply(y.flatten(), y_prev).mean()
	# take only the last parameters to avoid having the same members among the committee
	W, B = get_Params(y_prev) # take all the parameters for now !!!!
	w = W[0];
	layer_number=None
	...
	test_function = theano.function([x,y], error, updates=updates,
			allow_input_downcast=True)
	...
	for epoch in range(5):
		train_cost=[]
		for minibatch in range(n_train):
			x_value = x_train[minibatch*batch_size:(minibatch+1)*batch_size].reshape((batch_size, 3, 32, 32))
			y_value = y_train[minibatch*batch_size:(minibatch+1)*batch_size].reshape((batch_size, 1))
			train_function(x_value, y_value)
		for minibatch in range(n_train):
			x_value = x_train[minibatch*batch_size:(minibatch+1)*batch_size].reshape((batch_size, 3, 32, 32))
			y_value = y_train[minibatch*batch_size:(minibatch+1)*batch_size].reshape((batch_size, 1))
...
def build_training(lr=0.002, model=None):
	...
	#return train_function, valid_function, test_function, model, observation
