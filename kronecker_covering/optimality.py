...
from kronecker_factor import blocks_Fisher_P
...
def get_coeff_sample(query, dico_1, couple_keys):
	...
	for (key_input, key_output) in keys:
		if query[key_input].ndim==1:
			import pdb
			pdb.set_trace()
		c_n = np.trace( np.dot(dico_1[key_input],query[key_input]))
		d_n = np.trace( np.dot(dico_1[key_output], query[key_output]))
...
def opt_T(subsets, data_labelled, data_unlabelled, model, filters_info, size):
	...
	indices = greedy_selection(subsets, dico_l, dico_1, f_u, size, g, N)
	return indices
